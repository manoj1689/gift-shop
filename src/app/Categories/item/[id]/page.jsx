  ...
  uploadPhotosSpeaker.forEach((file, index) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // After form submission, reset the form fields including enterDetails
        resetForm();
      } else {
  ...
  const handleInputChange = (event) => {
    ...
    switch (name) {
      ...
      case "song":
        setSong(value)
        case "enterDetails":
          setEnterDetails([...value]); // Update the enterDetails state
          break;
          ...
          <Col sm={6}>
            ...
            <div>
              <Form onSubmit={handleFormSubmit}>
                ...
                <div>
                <div>
  {selectedProduct.enterDetails && selectedProduct.enterDetails.length >= 1 ? (
    <>
      {selectedProduct.enterDetails.map((ele, index) => (
        <div key={ele} className={styles.entryBox}>
          <Form.Control
            className={styles.enterDetails}
            type="text"
            placeholder={ele}
            name={`enterDetails[${index}]`} // Use an array notation for name
            value={formData[`enterDetails[${index}]`] || ""} // Access the value from formData using the array notation
          ...
          />
                ...
                <div className={styles.select}>
                  ...
                  type="checkbox"
                  id={id}
                  onChange={(event) => handleCheckboxChange(event, id)} // Pass the ID to the handler
                />
                            </div>
                ...
                <div className={styles.numberItemsBox}>
                  <span>Qty</span>
                  <span>
                    <Button className={styles.Button} variant="danger" onClick={decrement}>
                      -
                    </Button>
                  </span>
                  <span className={styles.number}>{number}</span>
                  <span>
                    <Button className={styles.Button} variant="danger" onClick={increment}>
                      +
                    </Button>
                  </span>
                </div>
                <Row>
                  <Col md={6}><Button href="/cartProducts"variant="danger" className={styles.addCart}>GO TO CART</Button>
                  </Col>
                <Col md ={6}>
                <Button type="submit" variant="danger" className={styles.buyButton} onClick={addToCart}>
                    <span>
                      <AddShoppingCartIcon />
                    </span>
                    <span>BUY NOW</span>
